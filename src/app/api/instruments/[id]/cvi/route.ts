/**
 * API route proxy untuk endpoint kalkulasi CVI instrumen backend.
 *
...
/**
 * Menangani GET /api/instruments/[id]/cvi — menghitung dan mengambil hasil CVI.
 *
 * @param _request - HTTP request dari client (tidak digunakan).
 * @param params - Params route dinamis berisi `id` instrumen.
 * @returns Hasil kalkulasi CVI atau pesan error.
 */
export async function GET(_request: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  const backendResp = await fetch(`${backendUrl}/api/v1/instruments/${id}/cvi`, {
