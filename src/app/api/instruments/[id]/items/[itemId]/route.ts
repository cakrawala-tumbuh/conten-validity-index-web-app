/**
 * API route proxy untuk endpoint item individual instrumen backend.
 *
...
/**
 * Menangani PATCH /api/instruments/[id]/items/[itemId] — memperbarui satu item.
 *
 * @param request - HTTP request berisi body update item.
 * @param params - Params route dinamis berisi `id` instrumen dan `itemId`.
 * @returns Data item yang sudah diperbarui atau pesan error.
 */
export async function PATCH(request: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  const backendResp = await fetch(`${backendUrl}/api/v1/instruments/${id}/items/${itemId}`, {
    method: "PATCH",
    headers: {
...
/**
 * Menangani DELETE /api/instruments/[id]/items/[itemId] — menghapus satu item.
 *
